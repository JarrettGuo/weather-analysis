...
def get_weather_list(city):
    ...
    if city in ['北京', '上海', '广州', '深圳']:
        logger.info('本地化查询')
        logger.info(f'local_weather={local_weather}')
...
def get_history_weather(city, start=6, end=0):
    ...
    params = {
        ...
        'appkey': '68472',
        'sign': '14dd1383ced32b3b59e312f7b2f46266',
        'format': 'json',
    }
    logger.info(params)
    try:
...
def show_rainfall_chart_data():
    ...
    if city in citys:
        ...
        return jsonify([{'date': item['date'], 'value': item['jyl']} for item in data[city]])
    logger.info('当前仅支持城市：北上广深')
    return []
...
def show_wtHumi_data():
    ...
    return jsonify([{'label': '空气湿度(%)', 'value': int(res[0]['wtHumi'])},
                   {'label': '其他', 'value': 100-int(res[0]['wtHumi'])}]
                   )
...
def show_pm2point5_data():
    ...
    print(res)
    return jsonify([{'date': item['date'], 'value': item['wtAqi']} for item in res])
